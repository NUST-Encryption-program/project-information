...
（8）在弹出的系统属性的对话框中，单击“高级”选项卡，然后单击“环境变量”按钮，如图所示。
 ...
 （10）单击“新建”按钮，弹出“新建系统变量”对话框，在这里需要设置3个变量，分别为：JAVA_HOME、CLASSPATH和Path。其中，JAVA_HOME对应的变量值填写JDK的安装目录，CLASSPATH的变量值填写.;%JAVA_HOME%\lib;%JAVA_HOME%\lib\tools.jar，编辑Path变量，在变量值的最后输入%JAVA_HOME%\bin;%JAVA_HOME%\jre\bin;（注意原来Path的变量值末尾有没有分号，如果没有先输入分号再输入上面的代码）。如图所示。
